...
> **GitHub repo:** [MohammedK-04/discordia](https://github.com/MohammedK-04/discordia) (project codename *Discordia*; product / brand is **DSD**).
...
5. **Brand mark** — use the crew photo (`public/dsd-crew.png`), not a letter-on-black placeholder.  
...
8. **Mock UI first was intentional** — when adding backend, preserve the flows already prototyped in `app/page.tsx`.
...
```
DSD/
├── app/                 # Next.js App Router (current UI lives in page.tsx)
│   ├── layout.tsx
│   ├── page.tsx         # Mobile-first prototype (dashboard, sheets, calendar)
│   └── globals.css
├── public/
│   └── dsd-crew.png     # Brand / logo (group photo)
├── features/            # Future plug-in modules (manifest + UI per feature)
...
Target module shape (when splitting out of the prototype):

```
features/
  <slug>/
    manifest.json      # id, name, createdBy, contributors, status, surfacesOnNoticeBoard
    ...components / routes
```
...
- Brand: crew photo logo  

**Not implemented:** real auth, invites, persistence, Stripe, email, attribution component, module registry.
...
```bash
cd ~/Desktop/DSD   # or your clone path
npm install
npm run dev
```
...
#### Task 1 — Repo & hygiene
Clean README, MIT, `.env.example`, contribution guide, agree on `/features` + manifest convention.  
...
3. New modules under `/features/<slug>/` with `manifest.json`  
4. No secrets in git  
...
Add a committed `.env.example` with empty keys when Task 1/4 lands.
...
1. **Read this README** and the current UI in `app/page.tsx` / `app/globals.css` before redesigning flows.  
